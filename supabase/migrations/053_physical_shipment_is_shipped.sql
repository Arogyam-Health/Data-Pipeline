-- 053 — Require physical Shiprocket lifecycle evidence for canonical is_shipped.
...
create or replace function data_pipeline.has_physical_shipment_evidence(
  p_shipment_status text,
  p_current_status text,
  p_shipment_status_id text,
  p_current_status_id text,
  p_delivered_date text,
  p_scan_evidence boolean
)
returns boolean
language sql
immutable
as $$
  select
    coalesce(p_scan_evidence, false)
    or coalesce(p_shipment_status_id, '') in (
      '6', '7', '9', '10', '15', '16', '17', '18', '21',
      '25', '26', '32', '38', '39', '40', '41', '42', '43',
      '44', '45', '46', '51', '55', '78', '90'
    )
    or coalesce(p_current_status_id, '') in (
      '6', '7', '9', '10', '15', '16', '17', '18', '21',
      '25', '26', '32', '38', '39', '40', '41', '42', '43',
      '44', '45', '46', '51', '55', '78', '90'
    )
    or upper(trim(coalesce(p_shipment_status, ''))) in (
      'PICKED UP', 'SHIPPED', 'IN TRANSIT', 'OUT FOR DELIVERY',
      'REACHED AT DESTINATION HUB', 'MISROUTED', 'DELIVERED',
      'RTO INITIATED', 'RTO DELIVERED', 'RTO IN TRANSIT', 'RTO OFD',
      'RTO NDR', 'UNDELIVERED', 'REACHED BACK AT SELLER CITY',
      'RETURN IN TRANSIT', 'RETURN DELIVERED', 'RETURN PICKED UP'
    )
    or upper(trim(coalesce(p_current_status, ''))) in (
      'PICKED UP', 'SHIPPED', 'IN TRANSIT', 'OUT FOR DELIVERY',
      'REACHED AT DESTINATION HUB', 'MISROUTED', 'DELIVERED',
      'RTO INITIATED', 'RTO DELIVERED', 'RTO IN TRANSIT', 'RTO OFD',
      'RTO NDR', 'UNDELIVERED', 'REACHED BACK AT SELLER CITY',
    ...
    or nullif(btrim(p_delivered_date), '') is not null
$$;
...
scan_evidence as (
  select
    ...
    bool_or(
      coalesce(s.sr_status, '') in (
        '6', '7', '9', '10', '17', '18', '21', '38', '39',
        '40', '41', '42', '46', '78'
      )
      or upper(trim(coalesce(s.sr_status_label, ''))) in (
        'PICKED UP', 'SHIPPED', 'IN TRANSIT', 'OUT FOR DELIVERY',
        'REACHED AT DESTINATION HUB', 'MISROUTED', 'DELIVERED',
        'RTO INITIATED', 'RTO DELIVERED', 'RTO IN TRANSIT', 'RTO OFD',
        'RTO NDR', 'UNDELIVERED', 'REACHED BACK AT SELLER CITY'
      )
      or upper(trim(coalesce(s.status, ''))) in (
        '000-T-DL', 'RTO INITIATED', 'RTO IN TRANSIT'
      )
      or upper(trim(coalesce(s.activity, ''))) ~ '^(SHIPMENT INSCAN|SHIPMENT OUTSCAN|SHIPMENT OUTSCANNED TO NETWORK|IN TRANSIT -|DISPATCHED - OUT FOR DELIVERY|SHIPMENT DELIVERED|PICKUP COMPLETED|PICK UP SCAN ON FIELD|RTO |RETURN )'
    ) as has_physical_shipment_scan
...
shiprocket_canonical as (
  select
    ...
    data_pipeline.has_physical_shipment_evidence(
      o.shipment_status,
      o.current_status,
      o.shipment_status_id,
      o.current_status_id,
      o.delivered_date,
      se.has_physical_shipment_scan
    ) as has_physical_shipment_evidence,
    ...
    case
      ...
      when upper(trim(coalesce(o.shipment_status,''))) in ('PICKED UP', 'SHIPPED', 'REACHED AT DESTINATION HUB', 'MISROUTED', 'REACHED BACK AT SELLER CITY')
        or upper(trim(coalesce(o.current_status,''))) in ('PICKED UP', 'SHIPPED', 'REACHED AT DESTINATION HUB', 'MISROUTED', 'REACHED BACK AT SELLER CITY')
        then 'in_transit'
      else 'other'
    end as status_bucket,
...
joined as (
  select
    ...
    ps.has_physical_shipment_evidence,
    ps.etd,
    ps.current_ts,
...
with_delivery as (
  select
    p.*,
    (p.shiprocket_match_status = 'MATCHED' and p.has_physical_shipment_evidence) as is_shipped,
