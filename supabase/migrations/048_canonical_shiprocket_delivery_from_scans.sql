-- 048_canonical_shiprocket_delivery_from_scans.sql
-- Post-047: make SQL delivery outcome agree with the application resolver.

create schema if not exists data_pipeline;
create schema if not exists analytics;

create or replace function data_pipeline.is_return_shipment(
  p_is_return boolean,
  p_order_id text,
  p_return_awb_code text,
...
immutable
as $func$
  select coalesce(p_is_return, false)
    or coalesce(p_order_id, '') ~* '^R_'
    ;
$func$;

comment on function data_pipeline.is_return_shipment(boolean,text,text,text,text) is
'Classifies shipment direction only from separate-return evidence. Lifecycle RETURN/RTO statuses and return_awb_code describe outcome/return linkage on the original forward shipment and are not direction signals.';
...
scan_evidence as (
  select
    ...
    max(s.scan_date) filter (where
      ...
      or upper(coalesce(s.status, '')) like '%000-T-DL%'
      or upper(coalesce(s.activity, '')) like '%SHIPMENT DELIVERED%'
...
delivery_classified as (
  select
    ...
    case
      ...
      when left(p.shiprocket_delivered_date_raw, 10) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' then 'DELIVERED'
...
with_delivery as (
  select
    ...
    case
      when p.delivery_outcome = 'DELIVERED' and p.shiprocket_delivered_date_raw ~ '^\d{4}-\d{2}-\d{2}' then p.shiprocket_delivered_date_raw::timestamptz
