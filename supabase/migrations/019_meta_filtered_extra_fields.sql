-- 019_meta_filtered_extra_fields.sql
-- Add frequency/cpm/reach/delivery/budget/ends/attribution to filtered and range functions so dashboard can show Frequency etc.

-- Campaign for_range already has reach; add cpm/frequency/delivery/budget
...
returns table (
  ...
  website_purchases numeric, website_purchase_roas numeric,
...
language sql stable as $$
  select
    ...
    case when coalesce(sum(d.spend),0)>0 then sum(d.purchase_value)/nullif(coalesce(sum(d.website_purchases),0),0) else null end,
...
language sql stable as $$
  select
    ...
    case when coalesce(sum(a.spend),0)>0 and coalesce(sum(a.website_purchases),0)>0 then sum(a.purchase_value)/sum(a.website_purchases) else null end,
