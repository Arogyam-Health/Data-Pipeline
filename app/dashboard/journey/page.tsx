  ...
  return (
    <main className="journey-shell">
      ...
      <section className="journey-panel">
        ...
        <div className="efficiency-strip"><span>Spend <b>{money(profitTotals.spend)}</b></span><span>Orders <b>{number(profitTotals.orders)}</b></span><span>AOV <b>{Number(profitTotals.orders) ? money(Number(profitTotals.orderRevenue) / Number(profitTotals.orders)) : "—"}</b></span><span>Shopify revenue <b>{money(profitTotals.orderRevenue)}</b></span><span>Delivered revenue <b>{money(profitTotals.deliveredRevenue)}</b></span><span>Meta ROAS <b>{number(profitTotals.metaRoas)}</b></span><span>Shopify ROAS <b>{number(profitTotals.shopifyRoas)}</b></span><span>Delivered ROAS <b>{number(profitTotals.deliveredRoas)}</b></span></div>
        <div className="table-scroll"><table className="profit-table"><thead><tr><th>{level}</th><th>Spend</th><th>Impressions</th><th>Clicks</th><th>LPV</th><th>Orders</th><th>Revenue</th><th>Shipped</th><th>Delivered</th><th>RTO</th><th>NDR</th><th>Delivered revenue</th><th>Meta ROAS</th><th>Shopify ROAS</th><th>Delivered ROAS</th><th>Hierarchy consistency</th></tr></thead><tbody>{profitRows.slice(0, 100).map((row, index) => <tr key={`${row.campaign_id}-${row.adset_id}-${row.ad_id}-${index}`} onClick={() => drill(row)}><td><strong>{text(level === "campaign" ? row.campaign_name : level === "adset" ? row.adset_name : row.ad_name)}</strong><small>{text(level === "campaign" ? row.campaign_id : level === "adset" ? row.adset_id : row.ad_id)}</small></td><td>{money(row.spend)}</td><td>{number(row.impressions)}</td><td>{number(row.clicks)}</td><td>{number(row.landing_page_views)}</td><td>{number(row.orders)}</td><td>{money(row.order_revenue)}</td><td>{number(row.shipped)}</td><td>{number(row.delivered)}</td><td>{number(row.rto)}</td><td>{number(row.ndr)}</td><td>{money(row.delivered_revenue)}</td><td>{number(row.meta_roas)}</td><td>{number(row.shopify_roas)}</td><td>{number(row.delivered_roas)}</td><td>{row.attribution_coverage == null ? "—" : `${(Number(row.attribution_coverage) * 100).toFixed(1)}% hierarchy-consistent`}</td></tr>)}</tbody></table></div>
      </section>
      ...
      {detail && <div className="journey-modal-backdrop" onClick={() => setDetail(null)}><aside className="journey-modal" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setDetail(null)}>×</button><h2>{text(detail.order.order_name)} — complete journey</h2><p className="modal-subtitle">Only events and dates present in source data are shown.</p>
        <div className="detail-grid">
          ...
          <section><h3>Shopify commerce</h3><dl><dt>Order ID</dt><dd>{detail.order.shopify_order_id}</dd><dt>Value</dt><dd>{money(detail.order.ordered_revenue)}</dd><dt>Payment</dt><dd>{text(detail.order.payment_type)}</dd><dt>Financial</dt><dd>{text(detail.order.financial_status)}</dd><dt>Fulfilment</dt><dd>{text(detail.order.fulfillment_status)}</dd><dt>Customer ID</dt><dd>{text(detail.order.customer_key)}</dd></dl></section>
